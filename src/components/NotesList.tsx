      ...
      {menuState &&
        createPortal(
          <div
            ref={menuRef}
            role="menu"
            className="fixed z-50 min-w-[190px] rounded-md border border-gray-200 bg-white p-1 shadow-lg animate-in fade-in-0 zoom-in-95"
            style={{ left: adjustedMenuState?.x ?? menuState.x, top: adjustedMenuState?.y ?? menuState.y }}
          >
            <button type="button" role="menuitem" className="w-full rounded-sm px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100" onClick={() => runAction('open')}>
              Open
            </button>
            <button type="button" role="menuitem" className="w-full rounded-sm px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100" onClick={() => runAction('open-new')}>
              Open in New Tab
            </button>
            <button type="button" role="menuitem" className="w-full rounded-sm px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100" onClick={() => runAction('edit-title')}>
              Edit Title
            </button>
            ...
            {view !== 'trash' ? (
              <>
                ...
                <button type="button" role="menuitem" className="w-full rounded-sm px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100" onClick={() => runAction('pin')}>
                  {menuState.note.isPinned ? 'Unpin' : 'Pin'}
                </button>
                <button type="button" role="menuitem" className="w-full rounded-sm px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100" onClick={() => runAction('archive')}>
                  {menuState.note.isArchived ? 'Restore from Archive' : 'Archive'}
                </button>
                <div className="my-1 h-px bg-gray-100" />
                <button type="button" role="menuitem" className="w-full rounded-sm px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50" onClick={() => runAction('delete')}>
                  Move to Trash
                </button>
              </>
            ) : (
              <>
                <div className="my-1 h-px bg-gray-100" />
                <button type="button" role="menuitem" className="w-full rounded-sm px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100" onClick={() => runAction('restore')}>
                  Restore
                </button>
                <button type="button" role="menuitem" className="w-full rounded-sm px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50" onClick={() => runAction('delete-forever')}>
                  Delete Permanently
                </button>
              </>
            )}
